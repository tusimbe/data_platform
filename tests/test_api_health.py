...
def test_health_returns_status(client, mocker):
    ...
    mocker.patch(
        "src.api.routes.health._check_redis", return_value={"status": "healthy", "latency_ms": 1.0}
    )
    mocker.patch(
        "src.api.routes.health._check_celery",
...
def test_health_has_components(client, mocker):
    ...
    resp = client.get("/api/v1/health/detail", headers={"X-API-Key": "test-api-key"})
    data = resp.json()
...
def test_health_redis_healthy(client, mocker):
    """Redis healthy 时应在组件中反映（需认证）"""
    mocker.patch(
        "src.api.routes.health._check_redis", return_value={"status": "healthy", "latency_ms": 2.5}
    )
    mocker.patch(
        "src.api.routes.health._check_celery",
        return_value={"status": "healthy", "workers": 1, "latency_ms": 5.0},
