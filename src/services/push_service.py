...
from src.core.exceptions import NotFoundError, NotImplementedError_, ServiceUnavailableError
...
def execute_push(
    ...
    if not connector_model:
        raise NotFoundError(
            f"No enabled connector found for type: {connector_type}",
        )
    ...
    try:
        ...
        raise ServiceUnavailableError(f"Connector unavailable: {str(e)}")

    try:
        result = connector.push(entity, records)
        return result
    except ConnectorError as e:
        raise ServiceUnavailableError(f"Push failed: {str(e)}")
    finally:
        try:
